...
public class RegisterServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String sqlQuery = "select * from " + DBUtils.TABLE_PASSWORD + " where userAccount = ?";
            ...
            result = preparedStatement.executeQuery(); // 先查询同样的账号（比如手机号）是否存在
            if(result.next()){ // 已存在
                resCode = "201";
                resMsg = "该账号已注册，请使用此账号直接登录或使用其他账号注册";
                userId = "";
            } else { // 不存在
                String sqlInsertPass = "insert into " + DBUtils.TABLE_PASSWORD + "(userAccount,userPassword) values('"+account+"','"+password+"')";
                // 更新类操作返回一个int类型的值，代表该操作影响到的行数
                int row1 = preparedStatement.executeUpdate(sqlInsertPass); // 插入帐号密码
                if(row1 == 1){
                    String sqlQueryId = "select userId from " + DBUtils.TABLE_PASSWORD + " where userAccount='" + account + "'";
                    ResultSet result2 = preparedStatement.executeQuery(sqlQueryId); // 查询新增记录的userId
                    if(result2.next()){
                        userId = result2.getInt("userId") + "";
                    }
                    String sqlInsertInfo = "insert into " + DBUtils.TABLE_USERINFO + "(userId) values('" + userId + "')";
                    int row2 = preparedStatement.executeUpdate(sqlInsertInfo); // 在用户信息表中插入刚注册的Id
